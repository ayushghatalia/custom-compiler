...
class TetrisEngine:
	def __init__(self, height=20, width=10, update_duration=100, move_down_duration=1000, background_col=0, foreground_col=0, difficulty=1, r1=1, r2=20, prob=1):
		self.window = tk.Tk()											  # fixed
		self.window.title("Python Text Tetris")		  # Programmable, inconsequential
		self.extetromino_distribution = range(r1,r2)	# Programmable:
		...
		if(background_col==1):
			self.bgcol='white'
		if(background_col==2):
			self.bgcol='light gray'
		if(background_col==3):
			self.bgcol='dark gray'
		if(foreground_col==1):
			self.fgcol='red'
		if(foreground_col==2):
			self.fgcol='green'
		...
		self.text_area = tk.Text(										# Essential and partially programmable
															...
															bg=self.bgcol,		# programmable
															fg=self.fgcol,					# programmable
		...
		if(self.difficulty==1):
			self.move_down_duration = 1000
		if(self.difficulty==2):
			self.move_down_duration = 750	
	...
	def new_piece(self,piece=None):
		if(piece==None): self.piece = Shape(get_any_extetromino(self.extetromino_distribution,self.prob))
