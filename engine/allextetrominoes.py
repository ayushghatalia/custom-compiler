...
def get_any_extetromino(distribution=range(1,20),prob=1):
	'''
	...
	'''
	if(prob==2):
		random_number = numpy.random.randn()
		x=distribution[0]
		y=distribution[len(distribution)-1]
		mapped_value = int((x + (y - x) * ((random_number+1) / 2)))
		if(mapped_value<x):
			mapped_value=x
		if(mapped_value>y):
			mapped_value=y
		...
		# print(x)
		# print(y)
		return eval("Extetromino"+str(distribution[mapped_value-1]))


	# tmp=numpy.random.randint(len(distribution))
	# print(tmp)
